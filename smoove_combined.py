...
def add_to_sms_list(phone, list_id):
    ...
    # שלב 1: מצא את ה-ID של הקונטקט לפי טלפון
    resp = requests.get(
        f"{SMOOVE_BASE_URL}/Contacts/{phone}",
        headers=headers,
        timeout=30,
    )
    if resp.status_code != 200:
        print(f"  ⚠️ לא נמצא קונטקט לטלפון {phone}: {resp.status_code}")
        return False
    contact_id = resp.json().get("id")
    if not contact_id:
        return False

    # שלב 2: הוסף לרשימה דרך PATCH
    resp2 = requests.patch(
        f"{SMOOVE_BASE_URL}/Contacts/{contact_id}",
        headers=headers,
        json={"listId": int(list_id)},
        timeout=30,
    )
    if resp2.status_code not in (200, 201, 204):
        # נסה POST ישיר לרשימה
        resp3 = requests.post(
            f"{SMOOVE_BASE_URL}/Contacts",
            headers=headers,
            json={"cellPhone": phone, "listId": int(list_id)},
            timeout=30,
        )
        if resp3.status_code not in (200, 201, 204):
            print(f"  ⚠️ נכשל ({resp3.status_code}): {resp3.text[:100]}")
            return False
    return True
