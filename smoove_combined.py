...
def add_contact_to_sms_list(phone, list_id):
    """מוסיף קונטקט בודד לרשימה ב-Smoove דרך /v1/Contacts – מטריגר אוטומציות."""
    headers = {
    ...
    resp = requests.post(
        f"{SMOOVE_BASE_URL}/Contacts?updateIfExists=true",
        headers=headers,
        json=payload,
        timeout=30,
    )
    if resp.status_code not in (200, 201, 204):
        print(f"  ⚠️ נכשל ({resp.status_code}): {resp.text[:100]}")
        return False
    return True
...
def send_sms_reminders(missing_people, all_contacts_by_key):
    ...
    # שלח כל קונטקט בנפרד כדי לטריגר אוטומציות ב-Smoove
    total_sent = 0
    for list_id, phones in phones_per_list.items():
        ...
        for phone in phones:
            if add_contact_to_sms_list(phone, list_id):
                total_sent += 1
