...
def add_to_sms_list(contact_id, list_id):
    """מוסיף קונטקט לרשימת תזכורת SMS ב-Smoove."""
    headers = {
        "Authorization": f"Bearer {SMOOVE_API_KEY}",
        "Content-Type": "application/json",
    }
    # נסה כמה פורמטים שונים
    payloads = [
        [{"id": contact_id}],
        {"id": contact_id},
    ...
    for payload in payloads:
        ...
        if resp.status_code in (200, 201, 204):
            return True
        print(f"  ⚠️ ניסיון נכשל ({resp.status_code}): {resp.text[:100]}")
    return False
...
def send_sms_reminders(missing_people, all_contacts_by_key):
    ...
    for phone_key, person in missing_people.items():
        ...
        if not contact:
            continue
        contact_id = (contact.get("id") or contact.get("externalId") or
                      contact.get("contactId") or contact.get("Id"))
        if not contact_id:
            print(f"  ⚠️ לא נמצא ID לקונטקט: {person.get('first')} {person.get('last')}")
            continue
        for training in person["missing"]:
            list_id = sms_list_vars.get(training)
            if list_id:
                success = add_to_sms_list(contact_id, list_id)
                if success:
                    total_sent += 1
